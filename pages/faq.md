...
Mac:
First check that Intel VT-x is supported in CPU by copying and pasting the following command in the terminal. 'sysctl -a | grep machdep.cpu.features'. You will then see an output with bunch of entries. If there is "VMX" in the entries then the CPU supports Intel VT-x feature, but it still may be disabled. Install the [latest software updates](https://support.apple.com/en-us/HT203296) by apple to enable it.
