...
After you are done making your changes use the command `git commit -a`. Note: it’s important that you are on the new branch, if you work on an issue multiple days make sure you are on the correct branch with `git checkout branch-name` before committing. Once you have finished working on the issue and have committed your changes, sync your repo [as before](http://open-learning-exchange.github.io/#!pages/gitandrepositories.md) and then use the command `git push origin name_of_your_new_branch`. Now you are ready to create a pull request, please remember to include the issue it is solving (for example, if your pull request fixes the issue number 320, then add `#320` to your pull request).
