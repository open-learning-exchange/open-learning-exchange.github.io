...
** Note :** You will see some common names used in git (which you can change), such as `master: the name of the main branch`, `upstream: the location where you forked the repository from`, and `origin: your github`. Both `upstream and origin` are considered remote.
You need also to know that a repository may include many branches.
